...
DATA_PATH = "/home/disk2/disk/3/tjk/RingData/data_health.npy"
...
def generate_split_config(mode: str, all_p: List, test_p: Optional[Union[List, str]] = None):
    ...
    if mode == "5fold":
        ...
        if mode == "train":
            ...
            split_config.append({"train": train_p, "test": test_p})
        else:
...
def main(config_path):
    ...
    for split_config in split_configs:
        ...
        if "train" in split_config:
            ...
            train_dataset = load_dataset(config["data_preprocess"], all_data[split_config["train"]])
            train_loader = DataLoader(train_dataset, batch_size=config["batch_size"], shuffle=True)
            # TODO: does all methods requires both training set and valid set?
            trainer.fit(model, train_loader)
        # test model 
        test_dataset = load_dataset(config["data_preprocess"], all_data[split_config["test"]])
